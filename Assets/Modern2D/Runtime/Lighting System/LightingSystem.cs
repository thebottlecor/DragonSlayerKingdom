...
namespace Modern2D
{
	...
	public class LightingSystem : MonoBehaviour
	{
        ...
        [HideInInspector] public Cryo<bool> noBlending = new Cryo<bool>(true);
        ...
        [HideInInspector] public Cryo<bool> _2dLightsMask = new Cryo<bool>(false); 
		
        [HideInInspector] public Cryo<int> _2dLightsLayer = new Cryo<int>(0);
		...
		public void SetCallbacks()
		{
			...
			noBlending.onValueChanged = OnShadowSettingsChanged;
		...
		public void OnShadowSettingsChanged()
		{
            ...
            shadowsMaterial = !noBlending.value ? (Material)Resources.Load("Materials/shadow materials/StylizedShadows2DMat") : (Material)Resources.Load("Materials/shadow materials/StylizedShadows2DMat2");
		...
		public void OnPivotSettingsChanged() 
		{
            ...
            foreach (StylizedShadowCaster2D c in GameObject.FindObjectsByType<StylizedShadowCaster2D>(FindObjectsInactive.Include, FindObjectsSortMode.None))
            {
			...
			extendedUpdateThisFrame = true;
			UpdateShadows(Transform.FindObjectsByType<StylizedShadowCaster2D>(FindObjectsSortMode.None).ToDictionary(t => t.transform, t => t.shadowData.shadow));
		}
		...
		private void Update()
		{
            ...
            _shadowsMaterial.SetInt("_SrcBlend", (int)UnityEngine.Rendering.BlendMode.Zero);
            _shadowsMaterial.SetInt("_DstBlend", (int)UnityEngine.Rendering.BlendMode.One);
		...
		int UpdateShadowList(Dictionary<Transform, StylizedShadowCaster> shadows)
		{
			...
			foreach (var shadowPair in shadows)
			{
				...
				else
				{
                    ...
                    if (Application.isPlaying || extendedUpdateThisFrame) continue;
        ...
        public void GetSpotLights() 
		{
			...
			if (_2dLightsMask.value)
			{
                ...
                lightsArr = FindObjectsByType<Light2D>(FindObjectsSortMode.None).Where(l => l.lightType == Light2D.LightType.Point).ToArray();
